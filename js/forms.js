					...
					if (el.parent('div.jqselect').length < 1) {
						...
						var selectboxOutput = function() {
							...
							function doSelect() {
								...
								if (option.first().html() === '' && el.data('placeholder') !== '') {
									//li.first().hide();
								}
								...
								if (liSelected.length) {
									...
									if (option.first().html() != optionSelected.html()) {
								...
								var selText = li.filter('.selected').html();
								...
								// изменение селекта
								el.on('change.styler', function() {
									divText.html(option.filter(':selected').html()).removeClass('placeholder');
									li.removeClass('selected sel').not('.optgroup').eq(el[0].selectedIndex).addClass('selected sel');
									// добавляем класс, показывающий изменение селекта
									if (option.first().html() != li.filter('.selected').html()) {
										selectbox.addClass('changed');
									} else {
